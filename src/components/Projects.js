...
function ProjectCard({ project, idx, onViewDetails, forceFullWidth = false }) {
  ...
  const handleMouseMove = (e) => {
    ...
    // Parallax: map mouse position to a small offset (-8px to +8px)
    setImgOffset({
      x: ((x / width) - 0.5) * 16,
      y: ((y / height) - 0.5) * 10,
    });
  ...
  return (
    <motion.div
      ...
      className={`glass group relative flex flex-col justify-between h-full overflow-hidden hover:shadow-[0_24px_50px_rgba(0,0,0,0.5)] border border-glass-border ${getBorderColorClass()}`}
      style={{
        "--mouse-x": "0px",
        "--mouse-y": "0px",
      }}
    >
      ...
      <div className={`relative w-full overflow-hidden border-b border-glass-border bg-glass-hover ${forceFullWidth ? "h-72 sm:h-96" : "h-52"}`}>
        <Image
          src={project.icon}
...
export default function Projects() {
  ...
  // Show only 3: first one is featured (full-width), next two are side-by-side
  const visibleProjects = projects.slice(0, 3);
  const featuredProject = visibleProjects[0];
  const sideProjects = visibleProjects.slice(1);
  ...
  return (
    <section id="projects" className="py-[120px] relative overflow-hidden">
      ...
      <div className="max-w-[1100px] mx-auto px-8 relative z-[2]">
        ...
        <div className="mb-6">
          <ProjectCard project={featuredProject} idx={0} onViewDetails={setSelectedProject} forceFullWidth />
        </div>

        {/* Row 2: Two side-by-side projects */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sideProjects.map((project, idx) => (
            <ProjectCard key={idx} project={project} idx={idx + 1} onViewDetails={setSelectedProject} />
          ))}
        </div>

        {/* See More → goes to /projects page */}
        {projects.length > 3 && (
          <div className="flex justify-center mt-12">
            <Magnetic>
              <button
                onClick={() => router.push("/projects")}
                className="cursor-pointer font-syne font-bold text-xs border border-glass-border bg-glass hover:bg-glass-hover text-text px-8 py-3.5 rounded-xl transition-all duration-300 flex items-center gap-2 hover:border-accent/40"
              >
                See More Projects
